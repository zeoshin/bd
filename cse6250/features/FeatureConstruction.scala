...
object FeatureConstruction {
  ...
  def constructDiagnosticFeatureTuple(diagnostic: RDD[Diagnostic]): RDD[FeatureTuple] = {
     ...
     */
    diagnostic
      .map(x => ((x.patientID, x.code.toLowerCase), 1.0))
      .groupByKey()
      .map(t => (t._1, t._2.sum))
  }
  ...
  def constructMedicationFeatureTuple(medication: RDD[Medication]): RDD[FeatureTuple] = {
     ...
     */
    medication
      .map(x => ((x.patientID, x.medicine.toLowerCase), 1.0))
      .groupByKey()
      .map(t => (t._1, t._2.sum))
  }
  ...
  def constructLabFeatureTuple(labResult: RDD[LabResult]): RDD[FeatureTuple] = {
    ...
    labResult
      .map(x => ((x.patientID, x.testName.toLowerCase), (x.value, 1)))
  ...
  def constructDiagnosticFeatureTuple(diagnostic: RDD[Diagnostic], candidateCode: Set[String]): RDD[FeatureTuple] = {
     ...
     */
    diagnostic
      .filter(x => candidateCode.contains(x.code.toLowerCase))
      .map(x => ((x.patientID, x.code.toLowerCase), 1.0))
      .groupByKey()
      .map(t => (t._1, t._2.sum))
  }
  ...
  def constructMedicationFeatureTuple(medication: RDD[Medication], candidateMedication: Set[String]): RDD[FeatureTuple] = {
     ...
     */
    medication
      .filter(x => candidateMedication.contains(x.medicine.toLowerCase))
      .map(x => ((x.patientID, x.medicine.toLowerCase), 1.0))
      .groupByKey()
      .map(t => (t._1, t._2.sum))
  }
  ...
  def constructLabFeatureTuple(labResult: RDD[LabResult], candidateLab: Set[String]): RDD[FeatureTuple] = {
    ...
    labResult
      .filter(x => candidateLab.contains(x.testName.toLowerCase))
      .map(x => ((x.patientID, x.testName.toLowerCase), (x.value, 1)))
      .reduceByKey((x, y) => (x._1 + y._1, x._2 + y._2))
      .mapValues { case (sum, count) => (1.0 * sum) / count }
  }
  ...
  def construct(sc: SparkContext, feature: RDD[FeatureTuple]): RDD[(String, Vector)] = {
    ...
    val feature_name_map = feature
      .map(x => (x._1._2.toLowerCase))
      .distinct()
      .collect
      .zipWithIndex
      .toMap
    ...
    val result = feature
      .map {
        case ((id, feature_name), count) => (id, (feature_name_map(feature_name.toLowerCase).toInt, count.toDouble))
      }.groupByKey()
      .map(x => (x._1, x._2.toSeq.sortBy { case (a, b) => a }))
      .map(x => (x._1, Vectors.sparse(feature_name_map.size, x._2).asInstanceOf[Vector]))
